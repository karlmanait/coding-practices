    ...
    : m_index(0)
    {
        ...
        do
        {
            ...
            else if (input_line == "expected_output:")
            {
                ...
                m_solver[m_index] = new Bfs(m_height[m_index], m_width[m_index], m_input[m_index]);
...
private:
    ...
    Bfs* m_solver[MAX_NUM_CASES];
    int m_height[MAX_NUM_CASES];
    int m_width[MAX_NUM_CASES];
};

class BfsParameterizedTestFixture :public ::testing::TestWithParam<int>
{
};

StdinParser* stdin_data;

TEST_P(BfsParameterizedTestFixture, PerTestCase)
{
    int index = GetParam();
