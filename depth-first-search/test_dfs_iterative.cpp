    ...
    : m_index(0)
    {
        ...
        do
        {
            ...
            else if (input_line == "expected_output:")
            {
                ...
                m_solver[m_index] = new Dfs(m_height[m_index], m_width[m_index], m_input[m_index]);
...
private:
    ...
    Dfs* m_solver[MAX_NUM_CASES];
    int m_height[MAX_NUM_CASES];
    int m_width[MAX_NUM_CASES];
};

class DfsParameterizedTestFixture :public ::testing::TestWithParam<int>
{
};

StdinParser* stdin_data;

TEST_P(DfsParameterizedTestFixture, PerTestCase)
{
    int index = GetParam();
